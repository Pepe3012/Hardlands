...
import com.hardlands.common.menu.Menu;
...
public final class ScenarioOptionsMenu implements Menu {
    ...
    public void build(MenuInventory menu, Player player) {
        ScenarioModule scenarioModule = HardlandsPlugin.getInstance().getScenarioManager().get(this.type);
        List<Map.Entry<String, Option<?>>> options = List.copyOf(scenarioModule.getOptionHolder().getOptions().entrySet());

...
public final class CommandInitializer {
    ...
    private static void registerCompletions(Hardlands plugin, PaperCommandManager manager) {
        ...
        completions.registerCompletion("scenario_options", context -> {
            ...
            if (definition == null || !plugin.getScenarioManager().isScenarioRegistered(definition)) return List.of();

            return plugin.getScenarioManager().getRegisteredScenario(definition).getRegisteredOptions().keySet();
        });
    }

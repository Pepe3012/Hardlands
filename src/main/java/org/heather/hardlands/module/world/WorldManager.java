...
@ConfigBuilder(
        ...
        options = {
            ...
            @OptionDef(
                    type = Integer.class,
                    validators = Validator.Keys.NON_NEGATIVE,
                    name = "meetupShrinkTime"),
            @OptionDef(
                    type = Integer.class,
                    validators = Validator.Keys.NON_NEGATIVE,
                    name = "deathmatchShrinkTime"),
...
public final class WorldManager extends WorldManagerConfiguration {
    ...
    private final PregenerationManager pregenerationManager =
            new PregenerationManager(requireChunkyService());

    public void configure() {
        this.forEachEnabledWorld(
                (world, centerX, centerZ, survivalSize) -> {
        ...
        this.forEachEnabledWorld(
                (world, centerX, centerZ, survivalSize) ->
                        this.pregenerationManager.reviewAndAccept(
                                new PregenerationRequest(
                                        world.getName(), centerX, centerZ, survivalSize)));
    }
    ...
    private void shrinkWorldBorders(
            Option<Integer> targetSizeOption, Option<Integer> shrinkDurationMinutesOption) {
                        ...
                        worldName -> {
                            ...
                            if (world == null) {
                                throw new IllegalStateException(
                                        "Enabled world is not loaded: " + worldName);
                            }

...
import io.github.pepe3012.hardlands.core.data.json.JsonDataManager;

import java.nio.file.Path;

public final class PresetRepository {

    private record ConfigurationPreset(
    ...
    public void save(String name) {
        managerFor(name).write(new ConfigurationPreset(
    ...
    private JsonDataManager<ConfigurationPreset> managerFor(String name) {
        return new JsonDataManager<>(
                Hardlands.GSON,
                this.directory.resolve(name + ".json"),
                ConfigurationPreset.class
        );
    }
}
...
final class ConfigurationGenerator {
    ...
    ConfigurationGenerator(ProcessingEnvironment processingEnvironment) {
        this.processingEnvironment = processingEnvironment;
    }

    void generate(TypeElement annotated) throws IOException {
        ConfigBuilder annotation = annotated.getAnnotation(ConfigBuilder.class);
        if (annotation == null) return;

        String packageName = this.processingEnvironment.getElementUtils().getPackageOf(annotated).getQualifiedName().toString();
        String className = annotated.getSimpleName() + "Configuration";
        String qualifiedName = packageName + "." + className;
        String superclass = this.resolveSuperclass(annotation);

        try (Writer writer = this.processingEnvironment.getFiler().createSourceFile(qualifiedName, annotated).openWriter()) {
    ...
    private void writeOptions(Writer writer, TypeElement annotated, OptionDef[] options) throws IOException {
        ...
        for (OptionDef option : options) {
            ...
            try {
                ...
                this.reportError("Invalid option '" + fieldName + "': " + exception.getMessage(), annotated);
            }
        }
    }

    private void writeOption(Writer writer, OptionDef option, String fieldName, String key) throws IOException {
        TypeMirror type = this.resolveType(option);

        if (this.isType(type, LIST_TYPE)) {
            this.writeCollectionOption(writer, option, fieldName, key, LIST_TYPE, "registerList");
            return;
        }

        if (this.isType(type, SET_TYPE)) {
            this.writeCollectionOption(writer, option, fieldName, key, SET_TYPE, "registerSet");
            return;
        }
        ...
        writer.write("    public final Option<%s> %s = super.registerOption(\"%s\", %s.class%s);\n\n"
                .formatted(typeName, fieldName, key, typeName, validators));
    }

    private void writeCollectionOption(Writer writer, OptionDef option, String fieldName, String key, String collectionType, String registerMethod) throws IOException {
        TypeMirror elementType = this.resolveElementType(option);

        if (this.isVoidType(elementType)) {
            throw new IllegalArgumentException(this.simpleTypeName(collectionType) + " option requires elementType");
        }
        ...
        writer.write("    public final Option<%s<%s>> %s = super.%s(\"%s\", %s.class%s);\n\n"
                .formatted(collectionType, elementTypeName, fieldName, registerMethod, key, elementTypeName, validators));
    }

    private void writeMapOption(Writer writer, OptionDef option, String fieldName, String key) throws IOException {
        TypeMirror keyType = this.resolveKeyType(option);
        TypeMirror valueType = this.resolveValueType(option);

        if (this.isVoidType(keyType)) throw new IllegalArgumentException("Map option requires keyType");
        if (this.isVoidType(valueType)) throw new IllegalArgumentException("Map option requires valueType");
        ...
        writer.write("    public final Option<java.util.Map<%s, %s>> %s = super.registerMap(\"%s\", %s.class, %s.class%s);\n\n"
                .formatted(keyTypeName, valueTypeName, fieldName, key, keyTypeName, valueTypeName, validators));
    }

    private void writeConstructor(Writer writer, String className, String identifier) throws IOException {
    ...
    private boolean validateOption(TypeElement annotated, Set<String> fieldNames, Set<String> keys, String fieldName, String key) {
    ...
    private TypeMirror resolveClass(Class<?> type) {
        TypeElement element = this.processingEnvironment.getElementUtils().getTypeElement(type.getCanonicalName());

        if (element == null) throw new IllegalStateException("Unable to resolve type: " + type.getName());

        return element.asType();
    }

    private boolean isType(TypeMirror type, String qualifiedName) {
        return this.processingEnvironment.getTypeUtils().erasure(type).toString().equals(qualifiedName);
    }
    ...
    private String referenceTypeName(TypeMirror type) {
        String typeName = type.getKind().isPrimitive()
                ? this.processingEnvironment.getTypeUtils().boxedClass((PrimitiveType) type).getQualifiedName().toString()
                : type.toString();

        return typeName.startsWith("java.lang.") ? typeName.substring("java.lang.".length()) : typeName;
    }
    ...
    private void reportError(String message, TypeElement element) {
